...
describe("Test de shop rodri", () => {
  ...
  describe("test de users", () => {
    ...
    it("Confirmar la informacion del token del user first_name sea rodri", async () => {
      const result = await requester.get("/api/user/profile").set("Cookie", `authToken= ${authToken}`);
      expect(result.body.payLoad.first_name).to.be.equal("rodri");
    });
  });
});
